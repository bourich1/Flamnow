...
export default function ProjectCardPreview({
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[200px] w-full overflow-hidden bg-neutral-900 flex items-center justify-center">
        {cover_image ? (
          <>
      ...
      <div className="p-6 flex flex-col gap-4 relative z-20">
        <div className="flex justify-between items-center">
          <span
            className="text-[9px] font-bold uppercase tracking-widest px-2.5 py-0.5 rounded-full border font-mono"
            style={{
              color: color || '#ED3F27',
          ...
          <span className="text-[10px] text-white/40 font-mono font-bold uppercase">
            {year || '2026'}
          </span>
        </div>

        <div>
          <h2 className="text-xl font-black text-white uppercase tracking-tight font-display leading-tight">
            {title || 'Project Case Study Title'}
          </h2>
          {tagline && (
            <p className="text-[10px] font-medium italic text-white/50 mt-1">
              &ldquo;{tagline}&rdquo;
            </p>
          )}
        </div>
