      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">
        ...
        <div className="lg:col-span-7 flex flex-col gap-4 items-start">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[9px] font-bold uppercase tracking-widest text-primary bg-primary/10 border border-primary/20 px-2 py-1 rounded font-mono">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black uppercase tracking-tight text-white leading-[0.95] font-display">
            <span className="block">{title_1 || 'We Ignite'}</span>
            <span className="block text-transparent stroke-text" style={{ WebkitTextStroke: "1px rgba(255,255,255,0.15)" }}>
              {title_stroke || 'Untamed'}
            </span>
            <span className="block text-[#ED3F27]">{title_2 || 'Growth.'}</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-3 mt-2">
            <button
              type="button"
              className="rounded-lg bg-[#ED3F27] text-white font-bold uppercase tracking-widest text-[9px] px-6 py-3 flex items-center gap-1.5 transition-all"
            >
            ...
            <button
              type="button"
              className="rounded-lg border border-white/10 bg-transparent text-white font-bold uppercase tracking-widest text-[9px] px-6 py-3 transition-all"
            >
        ...
        <div className="lg:col-span-5 flex flex-col gap-4 relative">
          ...
          <div className="grid grid-cols-1 gap-3">
            ...
            {/* Speed Card */}
            <div className="bg-[#121212] border border-white/5 p-3 rounded-2xl flex items-center gap-3">
              ...
              <div className="overflow-hidden">
                <p className="text-[8px] font-bold text-white/40 uppercase tracking-widest truncate">{speed_card_title || 'Flagship App'}</p>
                <p className="text-[10px] font-bold text-white truncate font-mono mt-0.5">{speed_card_value || 'Load Speed: 0.4s'}</p>
                <p className="text-[7px] font-mono text-green-400 mt-0.5">{speed_card_sub || 'VITALS 100%'}</p>
              </div>
            </div>
            ...
            <div className="bg-[#121212] border border-white/5 p-3 rounded-2xl flex items-center gap-3">
              ...
              <div className="overflow-hidden w-full">
                <p className="text-[8px] font-bold text-white/40 uppercase tracking-widest truncate">{roi_card_title || 'Campaign ROI'}</p>
                <p className="text-[10px] font-black text-white truncate font-mono mt-0.5">{roi_card_value || 'Attributed: 4.8x'}</p>
              </div>
            </div>
            ...
            <div className="bg-[#121212] border border-white/5 p-3 rounded-2xl flex items-center gap-3">
              ...
              <div className="overflow-hidden">
                <p className="text-[8px] font-bold text-white/40 uppercase tracking-widest truncate">{ctr_card_title || 'Ads Performance'}</p>
                <p className="text-[10px] font-bold text-white truncate font-mono mt-0.5">{ctr_card_value || 'CTR Rate: 6.2%'}</p>
                <p className="text-[7px] font-mono text-cyan-400 mt-0.5">{ctr_card_sub || '+22% ABOVE BM'}</p>
              </div>
            </div>
